...
static ssize_t pmccntr_show(char *arg, size_t size);
static int pmccntr_modify(const char *arg, size_t size);

/* file ops */
static ssize_t pmuctl_read(struct file *f,
                           char __user *userbuf,
                           size_t count,
                           loff_t *ppos);
static ssize_t pmuctl_write(struct file *f,
                            const char __user *userbuf,
                            size_t count,
                            loff_t *ppos);
static long pmuctl_ioctl(struct file *f, unsigned int cmd, unsigned long arg);

static struct pmu_ctl_cfg pmu_ctls[PM_CTL_CNT] = {
    [PM_CTL_PMCCNTR] = {.name = "PMCCNTR",
                        .show = pmccntr_show,
                        .modify = pmccntr_modify},
    [PM_CTL_CNTKCTL] = {
        .name = "CNTKCTL", .show = pmcntkctl_show, .modify = pmcntkctl_modify}};
...
static struct miscdevice pmuctl_dev = {
    .minor = MISC_DYNAMIC_MINOR, .name = "pmuctl", .fops = &pmuctl_fops};
...
static ssize_t pmuctl_write(struct file *f,
  ...
  for (i = 0; i < PM_CTL_CNT; i++) {
    ...
    if (pmu_ctls[i].modify != NULL) {
      ...
      dev_err(pmuctl_dev.this_device, "PMU %s not modifiable\n",
              pmu_ctls[i].name);
      ret = -ENOTSUPP;
    }
    break;
...
static long pmuctl_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
  ...
  if (_IOC_DIR(cmd) & _IOC_READ)
    err = !pmu_access_ok(VERIFY_WRITE, (void __user *)arg, _IOC_SIZE(cmd));
  else if (_IOC_TYPE(cmd) & _IOC_WRITE)
    err = !pmu_access_ok(VERIFY_READ, (void __user *)arg, _IOC_SIZE(cmd));
